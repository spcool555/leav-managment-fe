...
import { X, Calendar, FileText, Upload, File, Edit } from 'lucide-react';
import api, { API_BASE_URL } from '../services/api';
import toast from 'react-hot-toast';

const EditLeaveModal = ({ isOpen, onClose, leave, onLeaveUpdated }) => {
  ...
  useEffect(() => {
    if (leave && isOpen) {
      ...
      setExistingDocument(leave.supporting_document || null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(`/leave/${leave.id}/edit`, formDataToSend, {
        headers: {
        ...
        <div className="flex justify-between items-center p-6 border-b">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Update your leave application</p>
            </div>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="mb-3 bg-green-50 border border-green-200 rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-green-900">Current Document</span>
                  </div>
                  <a
                    href={`${API_BASE_URL}/leave/document/${existingDocument}`}
                    target="_blank"
              ...
              <div className="space-y-1 text-center">
                ...
                <div className="flex text-sm text-gray-600">
                  <label className="relative cursor-pointer bg-white rounded-md font-medium text-primary-600 hover:text-primary-500 focus-within:outline-none">
                    <span>{selectedFile ? 'Change file' : existingDocument ? 'Replace document' : 'Upload a file'}</span>
                    <input
                      type="file"
                      className="sr-only"
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm text-yellow-800">
                ...
                <p>You can only edit this leave request while it's in pending status. Once approved or rejected, changes are not allowed.</p>
              </div>
            </div>
          </div>
